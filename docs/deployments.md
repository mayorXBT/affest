...
deployer account on 2026-09-08. The deployer key was removed from `.env` after
the Windows command wrapper echoed it during a failed role-wiring attempt.
Because the strategy manager and verifier store their administrator immutably,
...
The role-wiring transactions (`setExecutor` and `setAuthorizedCaller`) were
not submitted because the wrapper failed to parse the function argument. The
existing deployments cannot be safely wired by a different wallet: those
admin checks bind to the old deployer. Redeploy the CC3 admin-owned contracts
with the fresh wallet, then wire the new executor and verifier.
...
- `AffestStrategyManager.executor()` is currently zero.
- `AffestAttestationVerifier.authorizedCaller()` is currently zero.
